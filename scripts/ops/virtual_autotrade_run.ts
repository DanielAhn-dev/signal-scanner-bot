...
async function main() {
  ...
  console.log(`[virtual-autotrade] run started mode=${mode} maxUsers=${maxUsers ?? "default"}`);
  ...
  console.log("[virtual-autotrade] run summary");
  console.log(JSON.stringify(summary, null, 2));
}

main().catch((error) => {
  console.error("[virtual-autotrade] run failed:", error);
  process.exit(1);
});
