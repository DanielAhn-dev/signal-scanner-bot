...
function rsiWilder(c: number[], n = 14) {
  const r: number[] = [];
  let g = 0,
    l = 0;
  ...
  r.unshift(...Array(Math.max(0, c.length - r.length)).fill(NaN));
  return r;
}
function roc(c: number[], n: number) {
  return c.map((v, i) => (i >= n ? ((v - c[i - n]) / c[i - n]) * 100 : NaN));
}
...
function withTimeout<T>(p: Promise<T>, ms: number, label = "op") {
  return Promise.race([
    p,
    new Promise<T>((_, rej) =>
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  async function analyzeAndReply(
    ...
    const end = new Date(),
      start = new Date(end.getTime() - 420 * 24 * 60 * 60 * 1000);
