...
def resource_filename(package, resource):
    """Return the filesystem path to a resource inside an installed package.

    This is a minimal shim used when setuptools' pkg_resources isn't available.
    """
    spec = importlib.util.find_spec(package)
    if spec is None:
        raise ImportError(f"Package {package!r} not found")
    # Prefer package search locations (packages) over origin file
