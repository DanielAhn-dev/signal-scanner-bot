...
import {
  ...
  isPromotedExecutionCandidate,
  resolveStablePromotionPolicy,
} from "../../services/stablePromotionPolicy";
import {
...
export async function handleScanCommand(
  ...
  const executionPicks = finalPicks
    .filter((item) => {
      ...
      const hasStableTag = hasStableAccumulationTag({
        stableAccumulation: stable?.stableAccumulation,
        stableAccumulationDays: stable?.stableAccumulationDays,
      });
      return isExecutionCandidate({ score, hasStableTag, policy: stablePromotionPolicy });
    })
    .slice(0, 8);
  ...
  const watchlistPicks = finalPicks
    .filter((item) => {
      ...
      return !isExecutionCandidate({ score, hasStableTag, policy: stablePromotionPolicy }) && hasStableTag;
    })
    .slice(0, 8);
  ...
  const formatCandidateLine = (item: PullbackRow, idx: number, mode: "execution" | "watchlist") => {
    ...
    const promotedExecution = isPromotedExecutionCandidate({
      score: Number(s?.total ?? 0),
      hasStableTag: stableAccumulationTag,
    ...
    const promotionTag = mode === "execution" && Number(s?.total ?? 0) < 70 && promotedExecution ? " · 승격" : "";
  ...
  const msg = buildMessage([
    ...
    ...(personalLines.length > 0
      ...
      : []),
    section(
      `실행 후보 (score>=70${stablePromotionPolicy.enabled ? ` 또는 승격>=${stablePromotionPolicy.minScore}` : ""})`,
