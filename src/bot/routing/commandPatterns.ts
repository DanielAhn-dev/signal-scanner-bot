export const CMD = {
  ...
  OPSTRIGGER: /^\/(opsrun|cronrun|운영트리거)(?:\s+(.+))?$/i,
  PREMARKET: /^\/(premarket|장전플랜|직장인플랜|오늘주문|오늘계획)(?:\s+(.+))?$/i,
  ...
  DISCOVERY_PROFILE: /^\/(discoveryprofile|발굴전략)(?:\s+(.+))?$/i,
  DISCOVERY_COMPARE: /^\/(discoverycompare|발굴비교)(?:\s+(.+))?$/i,
