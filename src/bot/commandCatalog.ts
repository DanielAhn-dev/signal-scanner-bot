...
const CALLBACK_COMMAND_TEXT: Record<string, string> = {
  ...
  ranking: "/ranking",
  discovery: "/발굴",
  discoveryprofile: "/발굴전략",
  discoverycompare: "/발굴비교",
  feed: "/feed",
  kospi: "/kospi",
  kosdaq: "/kosdaq",
  etf: "/etf",
...
const HELP_SECTIONS = [
  ...
  },
  {
    ...
    lines: [
      ...
      "/다음섹터 — 수급 유입 섹터",
      "/종목 <섹터> — 섹터별 대표 종목",
      "/스캔 [섹터] [추세|매집|진입|세력|후보|확인|매수검토] — 메뉴 추가 없이 수동 탐색 (선행/점화/실행 별칭 지원)",
      "/스캔로그 [일수] — 최근 스캔 통과율/선정 추세",
      "/눌림목 — 눌림목 매집 후보",
    ],
  },
  {
    ...
    lines: [
      ...
      "/수급 [종목] — 외국인·기관 매매동향",
      "/뉴스 [종목] — 시장·종목 뉴스",
      "/발굴 [개수] — 파이어족 멀티배거 후보 TOP (기본 20)",
      "/발굴전략 [혼합|하이라이트|눌림목|멀티배거|백테스트] — 자동매매 발굴 소스 설정",
      "/발굴비교 [개수] — 소스별 후보 비교",
    ],
  },
  {
    ...
    lines: [
      ...
      "/자동사이클 점검 학습 — 수동 학습 후보 점검",
      "/자동사이클 실행 학습 — 가상계좌 수동 학습 실행",
...
const CORE_HELP_LINES = [
  "핵심 5단계 여정",
  "1) /brief — 오늘 브리핑",
  "2) /scan · /pullback — 후보 스캔 (추세/매집/진입 + 후보/확인/매수검토 단계 필터)",
  "3) /종목분석 · /재무 · /수급 — 종목 검증",
...
export const TELEGRAM_BOT_COMMANDS: TelegramBotCommand[] = [
  ...
  { command: "scanlog", description: "최근 스캔 로그 요약" },
  ...
  { command: "discovery", description: "멀티배거 후보 발굴" },
  { command: "discoveryprofile", description: "자동매매 발굴 소스 전략 설정" },
  { command: "discoverycompare", description: "발굴 소스 비교" },
