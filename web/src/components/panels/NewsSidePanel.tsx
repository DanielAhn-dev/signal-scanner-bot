            ...
            <tbody>
              {news.map((item, i) => (
                ...
                >
                  ...
                  <td className="xls-cell" style={{ fontSize: 11, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                    {item.title}
                  </td>
                </tr>
              ))}
              {Array.from({ length: Math.max(0, 30 - news.length) }, (_, i) => (
                <tr key={`e${i}`} className={`xls-row${(news.length + i) % 2 === 0 ? ' xls-row--even' : ''}`}>
                  <td className="xls-row-num">{news.length + i + 1}</td>
