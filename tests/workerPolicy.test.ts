...
test("resolveCommandCategoryFromMessageText: 주요 명령을 카테고리로 분류한다", () => {
  ...
  assert.equal(resolveCommandCategoryFromMessageText("/자동트리거 준비"), "opstrigger");
...
test("resolveCommandCategoryFromCallbackData: 주요 콜백을 카테고리로 분류한다", () => {
  ...
  assert.equal(resolveCommandCategoryFromCallbackData("cmd:opstrigger:ready"), "opstrigger");
...
test("resolveJobTimeoutByCategory: 카테고리별 타임아웃을 반환한다", () => {
  ...
  assert.equal(resolveJobTimeoutByCategory("opstrigger"), 55000);
...
test("resolveWorkerTimeoutsFromEnv: env 값으로 카테고리별 타임아웃을 덮어쓴다", () => {
  const timeouts = resolveWorkerTimeoutsFromEnv({
    ...
    WORKER_JOB_TIMEOUT_OPSTRIGGER_MS: "59000",
  ...
  assert.equal(resolveJobTimeoutByCategory("opstrigger", timeouts), 59000);
