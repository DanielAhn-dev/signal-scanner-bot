...
test("resolveWatchDecision: STOP_LOSS 신호는 트리거가 없어도 우선 실행", () => {
  ...
  assert.equal(decision.blockedStopLoss, undefined);
});
...
test("resolveWatchDecision: TAKE_PROFIT 신호 + 트리거 없음이면 HOLD로 억제", () => {
  const decision = resolveWatchDecision({
    close: 10900,
    buyPrice: 10000,
    plan: basePlan,
