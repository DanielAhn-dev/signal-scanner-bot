...
## 6) 데이터 품질 연동 보수화 (2026-06-02)

월요일 신규 진입 후보 선정(`selectMondayCandidates`)은 점수 기준일 + 수급 기준일 신선도를 함께 평가합니다.

동작 요약
- `HIGH`: 정상 진입
- `MEDIUM`: 진입 수 60% 축소, 최소 진입 점수 +4 상향
- `LOW`: 신규 진입 차단

차단/보수화 기준
- 점수 기준일 2영업일 이상 지연: 신규 진입 차단
- 수급 기준일 확인 불가 또는 6영업일 이상 지연: 신규 진입 차단
- 수급 기준일 3영업일 이상 지연: 진입 수 축소 + 최소점수 상향

운영 확인 포인트
- `virtual_autotrade_runs.summary.notes`의 `guardNote`에 데이터품질 밴드/점수/차단 사유가 기록됩니다.
- 데이터 품질이 낮은 날에 신규 진입 건수가 줄거나 0건으로 떨어지는지 주간 점검합니다.
