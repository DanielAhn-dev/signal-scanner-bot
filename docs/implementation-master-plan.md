...
### BLOCK 6 — 실전 운용 일관화 (신규)

> 종목 선별 정확도보다 먼저, 매수·보유·자동점검에서 같은 기준이 반복되도록 고정한다.

#### ✅ 6-1. 보유/관심/자동점검 투자계획 계산 기준 통일 (완료: 2026-04-18)
- **목표**: `/매매`와 `/관심`, `/관심대응`, `/보유`, `/자동매도점검`, `/보유대응`이 동일한 팩터 기반으로 `buildInvestmentPlan()`을 계산
- **현황**: 완료. watchlist 계열 명령에서 최신 `scores.factors` 스냅샷을 우선 사용하고, 기준가 차이 보정을 거친 뒤 동일 투자계획 엔진으로 계산
- **작업(완료)**:
  - `watchlist.ts`에 점수 스냅샷 기반 투자계획 생성 헬퍼(`buildPlanFromScoreSnapshot`) 추가
  - 관심/보유/자동점검/보유대응 경로에서 `fetchLatestScoresByCodes()` + `scaleScoreFactorsToReferencePrice()` 경유로 계산 통일
  - 점수 미존재 시 기존 RSI/기준가 fallback 유지 (회귀 방지)
...
- **예정 파일**: `src/lib/investPlan.ts`, `src/bot/commands/buy.ts`
