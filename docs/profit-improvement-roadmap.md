...
## 2026-09-23 진단·개선 및 이후 운영 루프

### 진단 요약
- 7/11~9/21 보유종목 매도판단이 신선도 가드 조회 오류로 44영업일 중단 → 032830 -28%까지 방치 (062bd4a 수정)
- 스윕(CD금리 ETF)과 현금하한 충돌로 신규매수가 구조적으로 막혀 자금 대부분이 유휴
- 200일선 하락장 게이트가 지수 프록시 일봉 부족(23행)으로 한 번도 작동하지 않음
- 청산 백테스트: 수익잠금 트레일링 + 손절 확장 상한만 일관 개선, 손익비 강제는 악화(미채택)
- 진입 백테스트: 개별 팩터 조합은 학습/검증 분할마다 뒤집힘. 시장 추세(50일선)만 일관 → 50일선 하방 시 신규 매수 규모 50%
- pykrx 지수 API 고장 → 거래일 달력 폴백이 매 배치 수십 회 KRX 호출(차단 위험) → 종목 기간조회 1회로 교체

### 학습 데이터 원칙
- 원장은 지우지 않는다. 오염 구간은 `DATA_CONTAMINATION_WINDOWS`(virtualAutoTradeSelection.ts)에 등록하고 통계·분석에서 격리
- 적응형 통계는 2026-09-23 이후 거래만 사용 (그 전 깨끗한 실매매 표본은 1건)
- scores에는 TS 엔진(전체 팩터)과 Python 폴백(일부 팩터) 행이 섞여 있음 → 학습/백테스트는 전체 팩터 행만 사용

### 일봉 수집
- 정규 수집은 최근 10일을 함께 재조회(종목당 호출 수 동일)해 전날 부분 누락을 자가복구
- 오래된 종목 단위 구멍은 `heal_stock_daily_gaps`가 하루 25종목(AUTO_GAP_HEAL_MAX_TICKERS)씩 0.6초 간격으로 복구
- KRX 차단이 오면 대안: KIS 일봉 API(inquire-daily-itemchartprice, 이미 수급 수집에 KIS 키 사용 중)

### 이후 점검 루프
| 시점 | 할 일 | 명령 |
|------|------|------|
| 매주 | 스킵 사유 분포, 가드 경보 유무 확인 | `pnpm ops:skip-reasons` |
| 매주 | 실매매 승률/손익 (오염구간 자동 제외) | `pnpm dlx tsx scripts/analyze_winloss.ts --days 60` |
| 매도 20건 누적 후 | 적응형 게이트가 실제로 켜지는지 확인 (최소 표본 도달) | 자동사이클 notes의 `전략게이트` |
| 1~2개월 후 | 진입 신호 재검증 (하락장 표본이 IS에도 들어오도록 split 조정) | `pnpm dlx tsx scripts/backtest_entry_signals.ts --split=YYYY-MM-DD` |
| 청산 규칙 변경 전 | 청산 규칙 비교 | `pnpm dlx tsx scripts/backtest_exit_params.ts --grid=true` |
