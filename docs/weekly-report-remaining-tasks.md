...
- [x] 수직정렬 공식 통일 및 ReportTheme 재설계 최종 점검
...
- pdf-lib/fontkit 호환성 이슈로 현재는 Pretendard TTF 우선(없으면 Noto OTF fallback) 정책 사용
- weeklyReportService.ts는 현재 타입 오류 없음
- preview_economy_report.pdf 로컬 생성 확인 완료
- 다음 작업 시작 파일:
  - src/services/weeklyReportService.ts
  - scripts/previewReport.ts
